...
class LSTMAggregator(Node):
    ...
    def __init__(self):
        ...
        super().__init__("lstm_aggregator")

        self.n_subscribers = 14
    ...
    def aggregate_with_filter(self):
        ...
        for i in range(1, self.n_subscribers + 1):
            msg_filter.append(message_filters.Subscriber(self, String, f"classifier_node_{i}",qos_profile=1))

        self.ts = message_filters.ApproximateTimeSynchronizer(msg_filter, 1, 2.5, allow_headerless=True)
...
def threshold_prediction(output_list, threshold=0.65, n_nodes=0):
    ...
    threshold_output = ()
    arr_class = [i[0] for i in output_list[11:]]
